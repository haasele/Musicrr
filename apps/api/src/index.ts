...
const app = new Elysia()
  ...
  .get("/", () => ({
    ok: true,
    service: "musicrr-api",
    hint: "Open web UI at http://localhost:3000"
  }))
  .get("/health", () => ({ ok: true }))
  .get("/media/track/:trackId/stream", async ({ params, request }) => {
  ...
  .get("/media/track/:trackId/cover", async ({ params }) => {
    const track = await getTrackById(params.trackId);
  ...
  .post(
    ...
    ({ body }) => {
      ...
      return sql`SELECT id, password_hash, is_admin FROM users WHERE email = ${email} LIMIT 1`
        .then(async (users) => {
          const user = users[0] as { id: string; password_hash: string | null; is_admin: boolean } | undefined;
          if (!user?.password_hash) return { error: "User not found" };
  ...
  .post("/auth/logout", async ({ body }) => {
    await sql`DELETE FROM sessions WHERE id = ${body.sessionId}`;
    await cache.del(`session:${body.sessionId}`).catch(() => {});
    return { ok: true };
  ...
  .get("/library/tracks/:userId", async ({ params, query }) => {
  ...
  .get("/library/artists/:userId", ({ params }) =>
    sql`SELECT artist, COUNT(*)::int as count FROM tracks WHERE user_id = ${params.userId} GROUP BY artist ORDER BY artist ASC`
  )
  .get("/library/albums/:userId", ({ params }) =>
    sql`SELECT album, artist, COUNT(*)::int as count FROM tracks WHERE user_id = ${params.userId} GROUP BY album, artist ORDER BY album ASC`
  )
  .get("/library/search/:userId", async ({ params, query }) => {
